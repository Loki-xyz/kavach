...
class PrivilegeShield:
    ...
    # Patterns for detecting privileged content
    PATTERNS = {
        PrivilegeType.ATTORNEY_CLIENT: [
            ...
            r"my\s+lawyer\s+(?:said|advised|told)",
        ],
        PrivilegeType.WORK_PRODUCT: [
            r"work\s+product",
            r"litigation\s+(?:strategy|plan|work)",
            r"case\s+(?:strategy|theory|analysis)",
            r"prepared\s+(?:for|in\s+anticipation\s+of)\s+(?:litigation|trial)",
        ...
        PrivilegeType.CONFIDENTIAL: [
            ...
            r"NDA\s+(?:restricts|prohibits|requires)",
        ],
        ...
        PrivilegeType.FINANCIAL: [
            r"bank\s+(?:account|statement|details)",
            r"account\s+number",
            r"IFSC\s+code",
            ...
            for pattern in patterns:
                for match in pattern.finditer(text):
                    detection = PrivilegeDetection(
                        ...
                        confidence=self._calculate_confidence(ptype, match.group()),
                        suggested_redaction=self._get_redaction(ptype, match.group())
                    )
    ...
    def _calculate_confidence(self, ptype: PrivilegeType, text: str) -> float:
    ...
    def _get_redaction(self, ptype: PrivilegeType, text: str) -> str:
        """Get appropriate redaction text"""
        redactions = {
            PrivilegeType.ATTORNEY_CLIENT: "[ATTORNEY-CLIENT PRIVILEGED CONTENT REDACTED]",
            PrivilegeType.WORK_PRODUCT: "[WORK PRODUCT REDACTED]",
            PrivilegeType.CONFIDENTIAL: "[CONFIDENTIAL INFORMATION REDACTED]",
            PrivilegeType.PERSONAL_DATA: "[PERSONAL DATA REDACTED]",
            PrivilegeType.FINANCIAL: "[FINANCIAL DATA REDACTED]",
        }
        
        return redactions.get(ptype, "[REDACTED]")
    ...
    def generate_report(self, text: str, detections: List[PrivilegeDetection]) -> dict:
        ...
        if PrivilegeType.ATTORNEY_CLIENT in [d.privilege_type for d in detections]:
            report["recommendations"].append(
                "Attorney-client privilege detected. Ensure this content does not enter AI tools without redaction."
            )
        
        if PrivilegeType.PERSONAL_DATA in [d.privilege_type for d in detections]:
            report["recommendations"].append(
                "Personal data detected (Aadhaar/PAN/phone). Redact before sharing with AI tools to comply with data protection laws."
            )
